...
const SDKAppID = 1400813313;
const SecretKey = "3dcd65cb5ccbd9982f95b1a124a799fda6f3770d55c132171424222164c6d62b";
...
onMounted(() => {
  if (!getUrlParam("SDKAppID")) SDKAppID = 0;
  else {
    SDKAppID = parseInt(getUrlParam("SDKAppID") as string);
    isNewTab.value = true;
  }
  SecretKey = getUrlParam("SecretKey") || "";
...
async function startCall(typeString: string) {
  ...
  try {
    ...
    logReporter.callSuccess(SDKAppID, callType, typeString);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (error: any) {
