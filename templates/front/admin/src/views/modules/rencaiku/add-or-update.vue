<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="存入时间" prop="cunrushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.cunrushijian" 
                type="datetime"
                :readonly="ro.cunrushijian"
                placeholder="存入时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.cunrushijian" label="存入时间" prop="cunrushijian">
              <el-input v-model="ruleForm.cunrushijian" 
                placeholder="存入时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.cunrushijian = this.getCurDateTime()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongsimingcheng = true;
          ...
          if(o=='cunrushijian'){
            this.ruleForm.cunrushijian = obj[o];
	    this.ro.cunrushijian = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `rencaiku/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.rencaikuCrossAddOrUpdateFlag = false;
						 this.parent.search();
