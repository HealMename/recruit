...
def zhaopinxinxi_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        eval('''zhaopinxinxi.objects.filter({}='{}').update({}='{}')'''.format(username_str, username, password_str,
                                                                               init_pwd))
...
def zhaopinxinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __isAdmin__ == "是":
            ...
            if tablename != "users" and 'zhaopinxinxi'[:7] != 'discuss' and "userid" in zhaopinxinxi.getallcolumn(
                    zhaopinxinxi, zhaopinxinxi):
                req_dict["userid"] = request.session.get("params").get("id")
...
def zhaopinxinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        ...
        send_mail('用户注册', '您的注册验证码是【' + ''.join(code) + '】，请不要把验证码泄漏给其他人，如非本人请勿操作。', 'yclw9@qq.com', to,
                  fail_silently=False)

        cursor = connection.cursor()
        cursor.execute(
            "insert into emailregistercode(email,role,code) values('" + req_dict['email'] + "','用户','" + ''.join(
                code) + "')")
...
def zhaopinxinxi_autoSort2(request):
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d" % (
                request.session.get("params").get("id")) + " and tablename = 'zhaopinxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from zhaopinxinxi where $intelRecomColumn in ('%s" % ("','").join(
        ...
        return JsonResponse(
            {"code": 0, "msg": '', "data": {"currPage": 1, "totalPage": 1, "total": 1, "pageSize": 5, "list": L[0:6]}})
...
def zhaopinxinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM zhaopinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(
                xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM zhaopinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(
                xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM zhaopinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(
                xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def zhaopinxinxi_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django7681v/zhaopinxinxi/notify',
            # notify_url=''
        )
        pay_url = 'https://openapi.alipaydev.com/gateway.do?' + order_string
        pay_url = '<form name="punchout_form" method="post" action="{0}"><input type="hidden" name="biz_content" ><input type="submit" value="立即支付" style="display: none"></form>'.format(
            pay_url)
...
def zhaopinxinxi_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/django7681v/admin/dist/index.html#/zhaopinxinxi')
