...
def gongsi_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        records=gongsi.getbyparams(gongsi, gongsi, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)
...
def gongsi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =gongsi.page(gongsi, gongsi, req_dict, request)
...
def gongsi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'gongsi' order by addtime desc")
...
def gongsi_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django7681v/gongsi/notify',
            #notify_url=''
        )
...
def gongsi_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/django7681v/admin/dist/index.html#/gongsi')
