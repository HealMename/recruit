...
def gerenjianli_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        records=gerenjianli.getbyparams(gerenjianli, gerenjianli, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)
...
def gerenjianli_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'gerenjianli' order by addtime desc")
...
def gerenjianli_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django7681v/gerenjianli/notify',
            #notify_url=''
        )
...
def gerenjianli_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/django7681v/admin/dist/index.html#/gerenjianli')
